...
namespace Project_Zed
{
    static class Program
    {
        ...
        private static void LastHitSpell(Spell.Skillshot s)
        {
            if (s.IsReady())
            {
                foreach (var minion in from minion in Orbwalker.UnKillableMinionsList let predHealth = Prediction.Health.GetPrediction(minion, (int)(1000f * (s.SourcePosition ?? MyHero.Position).Distance(minion) / s.Speed) + s.CastDelay) where predHealth >= 0 where Damage(minion, s.Slot) >= predHealth select minion)
                {
                    CastQ(minion);
                }
                /*
